...
export class Camera {
    constructor(pos, up = [0, 1, 0], look_at = [0, 0, 0], near_plane = -1, far_plane = 1, vertical_fov = 90, horizontal_fov = 90) {
        this.pos = pos;
        this.up = up;
...
export function clone_cam(cam) {
    return Object.assign({}, cam);
}
// if just cam is passed, updates the camera basis
export function change_rendering_basis(cam, look_at = cam.look_at, pos = cam.pos, up = cam.up) {
    let up_vec = new THREE.Vector3().fromArray(up);
    let look_at_vec = new THREE.Vector3().fromArray(look_at);
    let pos_vec = new THREE.Vector3().fromArray(pos);
    let z = new THREE.Vector3().subVectors(pos_vec, look_at_vec).normalize();
...
function change_basis(cam, x, y, z) {
...
export function rotate_x(cam, degrees = 0) {
...
export function rotate_y(cam, degrees = 0) {
    let util_matrix = new THREE.Matrix4();
...
export function rotate_z(cam, degrees = 0) {
    let util_matrix = new THREE.Matrix4();
...
export function translate(cam, by) {
    let new_pos = cam.pos;
    new_pos[0] += by[0];
    new_pos[1] += by[1];
    new_pos[2] += by[2];
...
export function look_straight(cam) {
    let util_matrix = new THREE.Matrix4();
...
export function rotate_horizontal(cam, degrees = 0) {
    let new_cam = clone_cam(cam);
    let look_at = new THREE.Vector3().fromArray(cam.look_at);
    let pos = new THREE.Vector3().fromArray(cam.pos);
    let z = cam.z;
...
export function rotate_vertical(cam, degrees = 0) {
    let new_cam = clone_cam(cam);
    let look_at = new THREE.Vector3().fromArray(cam.look_at);
    let pos = new THREE.Vector3().fromArray(cam.pos);
    let z = cam.z;
...
export function rotate_clockwise(cam, degrees = 0) {
    let new_cam = clone_cam(cam);
    let z = cam.z;
    let x = cam.x;
    let y = cam.y;
