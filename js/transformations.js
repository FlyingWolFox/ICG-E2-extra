...
export var Transformation = THREE.Matrix4;
// returns [cos(), sin(), -sin(), cos()]
function create_rotate_submatrix(degrees) {
    let subm = [0, 0, 0, 0];
...
export function rotate_x(transform, degrees) {
...
export function rotate_y(transform, degrees) {
    let rotation = new THREE.Matrix4();
    let subm = create_rotate_submatrix(degrees);
...
export function rotate_z(transform, degrees) {
    let rotation = new THREE.Matrix4();
    let subm = create_rotate_submatrix(degrees);
    rotation.elements[0] = subm[0];
...
export function scale(transform, x = 1, y = 1, z = 1) {
...
// scales by n times
export function scale_byfactor(transform, factor = 1) {
    return scale(transform, factor, factor, factor);
}
// move to a coordinate
export function move(transform, to) {
...
// move by an amount
export function translate(transform, by) {
...
export function shear_x(transform, degrees_y = 0, degrees_z = 0) {
...
export function shear_y(transform, degrees_x = 0, degrees_z = 0) {
    let shear = new THREE.Matrix4();
...
export function shear_z(transform, degrees_x = 0, degrees_y = 0) {
    let shear = new THREE.Matrix4();
    degrees_x = degrees_x * (Math.PI / 180);
    degrees_y = degrees_y * (Math.PI / 180);
